...
Supply the [complete receiver coordinator](receiver-coordination-js.md) as `store`.
Its `ReceiverCoordinator` contract atomically runs `claimOnce` and
`reserveFunding`; `coordinateClaim` handles submission, ambiguity retention
and exact observation. Separate transfers can progress concurrently. Duplicate
events cannot repeat a claim or reuse its reserved input. `getSpendableVtxos`
only lists candidates. Your trusted `observeSpend` confirms the exact spend;
closing the subscription does not cancel an operation already submitting.
...
```ts
// receiver.ts
import { TaxiClient, type CovenantSpendConfig, type IncomingClaimTrust } from "@arkade-taxi/client";
import { ArkAddress, asset, type IWallet } from "@arkade-os/sdk";
import {
    coordinateClaim,
    receiverFundingCandidates,
    type ReceiverCoordinator,
    type ObserveSpend,
} from "./receiver-store.js";
...
export function receiveUsdt(options: {
    ...
    const { store, trusted, config } = options;
        ...
        for (const claim of batch.claims) {
            ...
            void coordinateClaim(
                ...
                async () => {
                    ...
                    const transfer = await taxi.verifyIncomingClaim(claim, expect, trusted, config);
                    const destination = ArkAddress.decode(claim.receiverAddress).pkScript;
                    const candidates =
                        options.mode === "recycle"
                            ? await receiverFundingCandidates(
                                  options.wallet,
                                  destination,
                                  transfer.value,
                              )
                    ...
                    const funding = candidates
                        ? await store.reserveFunding(operationId, candidates)
                        : undefined;
                    return {
                        funding,
                        submit: () =>
                            funding
                                ? taxi.recycle(transfer, funding, destination)
                                : taxi.purchase(transfer, destination),
                    };
                },
                options.observeSpend,
